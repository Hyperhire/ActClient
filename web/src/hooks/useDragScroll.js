...
const useDragScroll = (dragRef: RefObject<HTMLElement>) => {
  let isDown = false; // 사용자가 마우스를 눌렀는지 판단할 변수 선언
  let startX; // 사용자가 마우스를 누른 시작 지점 변수 선언
  let startScrollLeft; // 사용자가 마우스를 눌렀을 때 scrollLeft의 위치 변수 선언

  const mouseTouchDown = e => {
    // 마우스를 눌렀을 때 이벤트 함수
